...
//     }
//
//     getValidKeys(config).forEach((key) => this[`set${startCase(key)}`](config[key]));
//   }
//
//   setGridBreakPoints(config) {
//     const hasPermittedFields = Object.keys(config).every((key) => GRID_KEYS.includes(key));
//     const hasPermittedValues = Object.values(config).every((value) => Number.isInteger(value));
